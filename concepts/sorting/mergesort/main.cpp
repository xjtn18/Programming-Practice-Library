...
We will have a function that takes the list to sort and the range to sort (at first, this will be the entire range).
We will form the sorted list by recursively calling the same function to sort the first half or the list, and we
will do the same for the second half. Once we have these 2 sorted lists, we will merge them together, which is trivial.
The base case is when the halves of the arrays are of length 1. In this case, we will choose to either swap to values
if they aren't in order, or leave them as is if they are. We can now assume each sub-call will recursively sort each sublist,
and our final return when the call stack finished popping will contain the sorted version of the entire original list.
